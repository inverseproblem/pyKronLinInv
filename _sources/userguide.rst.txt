...
The **first** thing to compute is always the set of "factors" using the function :func:`calcfactors()`. 
Finally, the posterior mean (see :func:`posteriormean()`) and/or covariance (or part of it) (see :func:`blockpostcov()`) can be computed.

2D example
------------

An example of how to use the code for 2D problems is shown in the following. Notice that the code is written for a 3D problem, however, by setting some of the matrices as identity matrices with size of 1 :math:`\times` 1, a 2D problem can be solved without much overhead.
...
(:math:`\mathbf{C}_{\rm{M}}^{\rm{x}} \, , \mathbf{C}_{\rm{M}}^{\rm{y}} \, , \mathbf{C}_{\rm{M}}^{\rm{z}}`) and observed data (:math:`\mathbf{C}_{\rm{D}}^{\rm{x}} \, , \mathbf{C}_{\rm{D}}^{\rm{y}} \, , \mathbf{C}_{\rm{D}}^{\rm{z}}`) are simply an identity matrix of shape 1 :math:`\times` 1, since it is a 2D problem.
...
In order to solve the inverse problem using KronLinInv, we first need to compute the "factors" using the function :func:`calcfactors()`, which takes as inputs a set of arrays representing the covariance matrices and the forward operators.
...
We then construct some covariance matrices and a forward operator. The "first" covariance matrices for model parameters
(:math:`\mathbf{C}_{\rm{M}}^{\rm{x}} \, , \mathbf{C}_{\rm{M}}^{\rm{y}} \, , \mathbf{C}_{\rm{M}}^{\rm{z}}`) and observed data (:math:`\mathbf{C}_{\rm{D}}^{\rm{x}} \, , \mathbf{C}_{\rm{D}}^{\rm{y}} \, , \mathbf{C}_{\rm{D}}^{\rm{z}}`) are simply an identity matrix of shape 1 :math:`\times` 1, since it is a 2D problem.
The forward relation (forward model) is created from three operators (:math:`\mathbf{G}^{\rm{x}} \, , \mathbf{G}^{\rm{y}} \, , \mathbf{G}^{\rm{z}}`). Remark: the function :func:`mkCovSTAT()` used in the following example is *not* part of KronLinInv. See the 2D example for the code of :func:`mkCovSTAT()`.
